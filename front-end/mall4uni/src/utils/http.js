...
const http = {
  request: async function (params) {
    ...
    return new Promise((resolve, reject) => {
      // 构建请求头：登录和注册接口不需要 Authorization
      const headers = {}
      // 如果是登录类接口（params.login === true 或 URL 是 /login 或 /user/register），则不添加 Authorization
      const isLoginRequest = params.login === true || 
                            params.url === '/login' || 
                            params.url?.endsWith('/login') ||
                            params.url === '/user/register' ||
                            params.url?.endsWith('/user/register')
      if (!isLoginRequest) {
      ...
      uni.request({
        ...
        url: (() => {
          ...
          // 登录和注册接口不使用 /p/ 前缀，直接使用根路径
          const isLoginOrRegister = params.login === true || 
                                    params.url === '/login' || 
                                    params.url?.endsWith('/login') ||
                                    params.url === '/user/register' ||
                                    params.url?.endsWith('/user/register')
          ...
          if (isLoginOrRegister) {
            // 登录和注册接口直接使用完整 URL（相对于 API 服务根路径）
            return params.url || ''
          }
          
          // 其他接口使用 VITE_APP_BASE_API
          const baseUrl = import.meta.env.VITE_APP_BASE_API || ''
          const url = params.url || ''
          
          // 如果 URL 已经以 /p/ 开头，直接返回（避免双斜杠）
          if (url.startsWith('/p/')) {
            return url
          }
          
          // 如果 URL 已经以 /p 开头（但没有后面的斜杠），也直接返回
          if (url.startsWith('/p') && url.length > 2 && url[2] !== '/') {
            return url
          }
          
          // 处理双斜杠问题：如果 baseUrl 以 / 结尾，且 url 以 / 开头，则去掉一个斜杠
          if (baseUrl.endsWith('/') && url.startsWith('/')) {
            return baseUrl + url.substring(1)
          }
          // 如果 baseUrl 不以 / 结尾，且 url 不以 / 开头，则添加一个斜杠
          if (!baseUrl.endsWith('/') && !url.startsWith('/') && baseUrl && url) {
            return baseUrl + '/' + url
          }
          return baseUrl + url
  ...
  loginSuccess: (result, fn) => {
    ...
    if (!result.accessToken) {
      console.error('登录成功但缺少 accessToken')
      uni.showToast({
        title: '登录失败，缺少必要信息',
        icon: 'none'
      })
    ...
    // 计算过期时间，如果 expiresIn 存在则使用，否则使用默认值（24小时）
    const expiresIn = result.expiresIn || 86400 // 默认24小时（秒）
    const expiresTimeStamp = expiresIn * 1000 / 2 + new Date().getTime()
    // 缓存token的过期时间
