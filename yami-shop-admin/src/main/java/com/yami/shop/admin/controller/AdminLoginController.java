...
public class AdminLoginController {
    ...
    public ServerResponseEntity<?> login(
        ...
        log.info("收到登录请求，用户名: {}", captchaAuthenticationDTO.getUserName());
        ...
        if (!response.isSuccess()) {
            log.warn("验证码验证失败，用户名: {}", captchaAuthenticationDTO.getUserName());
            return ServerResponseEntity.showFailMsg("验证码有误或已过期");
        }
        log.debug("验证码验证成功");

        log.debug("开始查询用户信息，用户名: {}", captchaAuthenticationDTO.getUserName());
        SysUser sysUser = null;
        try {
            sysUser = sysUserService.getByUserName(captchaAuthenticationDTO.getUserName());
            log.debug("用户查询完成，结果: {}", sysUser != null ? "找到用户" : "未找到用户");
        } catch (Exception e) {
            log.error("查询用户信息时发生异常，用户名: {}", captchaAuthenticationDTO.getUserName(), e);
            throw e;
        }
        
        if (sysUser == null) {
            log.warn("用户不存在，用户名: {}", captchaAuthenticationDTO.getUserName());
        ...
        if (Objects.equals(sysUser.getStatus(),0)) {
            log.warn("用户已被禁用，无法登录，用户ID: {}, 用户名: {}", sysUser.getUserId(), sysUser.getUsername());
            // 未找到此用户信息
            throw new YamiShopBindException("未找到此用户信息");
        }

        log.debug("开始获取用户权限，用户ID: {}", sysUser.getUserId());
        Set<String> permissions = null;
        try {
            permissions = getUserPermissions(sysUser.getUserId());
            log.debug("用户权限获取完成，权限数量: {}", permissions != null ? permissions.size() : 0);
        } catch (Exception e) {
            log.error("获取用户权限时发生异常，用户ID: {}", sysUser.getUserId(), e);
            throw e;
        }
        ...
        userInfoInToken.setPerms(permissions);
        userInfoInToken.setNickName(sysUser.getUsername());
        userInfoInToken.setShopId(sysUser.getShopId());
        // 存储token返回vo
        log.debug("开始存储token，用户ID: {}", sysUser.getUserId());
        TokenInfoVO tokenInfoVO = tokenStore.storeAndGetVo(userInfoInToken);
        log.info("登录成功，用户ID: {}, 用户名: {}", sysUser.getUserId(), sysUser.getUsername());
