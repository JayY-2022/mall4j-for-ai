...
public class ProductController {
    ...
    public ServerResponseEntity<Product> info(@PathVariable("prodId") Long prodId) {
        ...
        // 将 actualStocks 映射到 stocks 字段，供前端编辑页面显示实际库存
        // stocks 字段在数据库中表示未付款订单数，但前端编辑页面需要显示实际库存
        for (Sku sku : skuList) {
            if (sku.getActualStocks() != null) {
    ...
    public ServerResponseEntity<String> update(@Valid @RequestBody ProductParam productParam) {
        ...
        // 处理 SKU 库存字段：将前端传入的 stocks 值保存到 actualStocks，保持 stocks 字段不变
        // 前端编辑页面使用 stocks 字段显示和编辑实际库存，但数据库中 stocks 是未付款订单数
        List<Sku> skuList = product.getSkuList();
        if (skuList != null) {
            // 创建数据库 SKU 的映射，用于查找原始 stocks 值
            Map<Long, Sku> dbSkuMap = dbSkus.stream()
                .collect(Collectors.toMap(Sku::getSkuId, sku -> sku));
            
            for (Sku sku : skuList) {
                // 将前端传入的 stocks 值（实际是实际库存）保存到 actualStocks
                if (sku.getStocks() != null) {
                    sku.setActualStocks(sku.getStocks());
                }
                // 保持 stocks 字段为数据库中的原始值（未付款订单数）
                Sku dbSku = dbSkuMap.get(sku.getSkuId());
                if (dbSku != null) {
                    sku.setStocks(dbSku.getStocks());
                } else {
                    // 新创建的 SKU，stocks 初始化为 0
                    sku.setStocks(0);
                }
            }
        }
