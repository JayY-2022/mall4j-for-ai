...
public interface ProductMapper extends BaseMapper<Product> {
    ...
    /**
     * 根据分类ID、商品名称和排序分页获取商品
     * @param page
     * @param categoryId 分类ID，可为null表示不限制分类
     * @param prodName 商品名称关键词
     * @param sort 排序方式：0-默认，1-销量，2-价格
     * @param orderBy 排序顺序：0-降序，1-升序
     * @return
     */
    IPage<SearchProdDto> getSearchProdDtoPageByCategoryAndName(Page page, @Param("categoryId") Long categoryId, @Param("prodName") String prodName, @Param("sort") Integer sort, @Param("orderBy") Integer orderBy);

    /**
     * 根据分类ID列表、商品名称和排序分页获取商品（支持父分类及其所有子分类）
     * @param page
     * @param categoryIds 分类ID列表，可为null或空表示不限制分类
     * @param prodName 商品名称关键词
     * @param sort 排序方式：0-默认，1-销量，2-价格
     * @param orderBy 排序顺序：0-降序，1-升序
     * @return
     */
    IPage<SearchProdDto> getSearchProdDtoPageByCategoryIdsAndName(Page page, @Param("categoryIds") List<Long> categoryIds, @Param("prodName") String prodName, @Param("sort") Integer sort, @Param("orderBy") Integer orderBy);
