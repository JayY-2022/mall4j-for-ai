    ...
    public ServerResponseEntity<String> addItem(@Valid @RequestBody ChangeShopCartParam param) {
        ...
        for (ShopCartItemDto shopCartItemDto : shopCartItems) {
            if (Objects.equals(param.getSkuId(), shopCartItemDto.getSkuId())) {
                ...
                // -1为无限库存，不需要检查库存
                // 使用 actualStocks（实际库存）而不是 stocks（未付款订单数）来检查
                Integer actualStocks = skuParam.getActualStocks();
                if (actualStocks != null && actualStocks != -1 && actualStocks < basket.getBasketCount() && shopCartItemDto.getProdCount() > 0) {
        ...
        // 使用 actualStocks（实际库存）而不是 stocks（未付款订单数）来检查
        Integer actualStocks = skuParam.getActualStocks();
        if (actualStocks != null && actualStocks != -1 && actualStocks < param.getCount()) {
            return ServerResponseEntity.showFailMsg("库存不足");
        }
        // 所有都正常时
