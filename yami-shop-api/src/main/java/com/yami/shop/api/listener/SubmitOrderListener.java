...
public class SubmitOrderListener {
    ...
    private Sku checkAndGetSku(Long skuId, ShopCartItemDto shopCartItem, Map<Long, Sku> skuStocksMap) {
        ...
        // -1为无限库存
        // 使用 actualStocks（实际库存）而不是 stocks（未付款订单数）来检查库存
        Integer actualStocks = sku.getActualStocks();
        if (actualStocks != null && actualStocks != -1 && shopCartItem.getProdCount() > actualStocks) {
            throw new YamiShopBindException("商品：[" + sku.getProdName() + "]库存不足");
        }

        if (actualStocks != null && actualStocks != -1) {
            Sku mapSku = new Sku();
            mapSku.setProdId(sku.getProdId());
            // 这里的库存是改变的库存（用于扣减 actual_stocks）
